...
{
#define fputs(a, b) ( sprintf((buf+=strlen(a)) - strlen(a), a) )
	int i;
	char nbuf[FSIZE+1];		/* one extra for terminating '\0' */
	unichar ubuf[FSIZE+1];	/* one extra for terminating '\0' */
	int ulen;
	struct names *p;
	...
	case 1:
		fputs("tar archive", stdout);
		return 1;
	case 2:
		fputs("POSIX tar archive", stdout);
		return 1;
	}
...
/*
	if (has_long_lines)
		fputs(", with very long lines", stdout);
*/
	/*
...
/*
	...
	if (n_crlf == 0 && n_cr == 0 && n_nel == 0 && n_lf == 0)
		...
		fputs(" line terminators", stdout);
	}

	if (has_escapes)
		fputs(", with escape sequences", stdout);
	if (has_backspace)
		fputs(", with overstriking", stdout);
*/
	return 1;
}
