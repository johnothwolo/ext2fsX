...
static int mprint	__P((union VALUETYPE *, struct magic *, char *buf));
static void mdebug	__P((int, char *, int));
	...
	int nbytes;
{
	...
	if (tmpoff == NULL)
		if ((tmpoff = (int *) malloc(tmplen = 20)) == NULL)
	...
	for (magindex = 0; magindex < nmagic; magindex++) {
		...
		tmpoff[cont_level] = mprint(&p, &magic[magindex], s);
		/*
...
mprint(p, m, buf)
	...
	struct magic *m;
        char *buf;
{
#define printf(a, b)  (sprintf(buf, a, b))
	...
	int t=0 ;
  	...
  	case LELONG:
		v = p->l;
		v = signextend(m, v) & m->mask;
		(void) printf(m->desc, (unsigned int) v);
	...
	default:
		fprintf(stderr, "invalid m->type (%d) in mprint().\n", m->type);
		/*NOTREACHED*/
	}
	return(t);
#undef printf
	...
	struct magic *m;
{
	...
	case LEDATE:
		...
		return 1;
	default:
		fprintf(stderr, "invalid type %d in mconvert().\n", m->type);
		return 0;
	}
}
	...
	struct magic *m;
{
	...
	default:
		fprintf(stderr, "invalid type %d in mcheck().\n", m->type);
		return 0;/*NOTREACHED*/
	}
	...
	default:
		matched = 0;
		fprintf(stderr, "mcheck: can't happen: invalid relation %d.\n", m->reln);
		break;/*NOTREACHED*/
	}
